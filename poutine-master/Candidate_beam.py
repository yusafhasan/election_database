...
def run():
     ...
     sql = 'SELECT cname, party FROM hdv_modeled.Candidate limit 50'
     ...
     formatted_cname_pcoll = query_results | 'Format CNAME' >> beam.ParDo(FormatCan())

     # write PCollection to log file
     # formatted_jname_pcoll | 'Write log 1' >> WriteToText('formatted_jname_pcoll.txt')

     # group candidates by (cname)
     grouped_cname_pcoll = formatted_cname_pcoll | 'Group by cname' >> beam.GroupByKey()

     # write PCollection to log file
     # grouped_jname_pcoll | 'Write log 2' >> WriteToText('grouped_jname_pcoll.txt')

     # remove duplicate candidate records
     distinct_can_pcoll = grouped_cname_pcoll | 'Dedup candidate records' >> beam.ParDo(DedupCanRecords())
     # write PCollection to log file
     # distinct_jur_pcoll | 'Write log 3' >> WriteToText('output.txt')

     dataset_id = 'hdv_modeled'
     table_id = 'Candidate_Beam'
